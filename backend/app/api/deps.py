...
are re-exported below. **Supabase remains the identity provider.** Nothing here
issues, refreshes or stores a credential — these dependencies verify a token
Supabase already signed and hand the route an `AuthContext`. There is no second
user system and no password anywhere in this backend.
"""
